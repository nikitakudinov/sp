...
class SupabaseSupabaseUser extends BaseAuthUser {
  SupabaseSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> supabaseSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SupabaseSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
